...
def get_ptm_from_name(ptm_name, ptm_path,vocab,pool_method,word_dropout=0.01,**kwargs):
...
@torch.no_grad()
def simulate_ee(old_pred,ee_mode,win_size,threshold,t_level_t,device,constrain_both,num_hidden_layers=12,criterion='entropy'):
    '''
    ...
    threshold = list(map(lambda x:min(x,0.3),threshold))
...
def inverse_pull_word_to_wordpiece(token_exit_layers,word_to_wordpiece_num,num_hidden_layers):
    '''
...
class Sample_Stop_Update_Callback(Callback):
    def __init__(self,ee_mode,sandwich_samll,sandwich_full,device,constrain_both,num_hidden_layers,true_copy,
                 min_win_size,max_win_size,min_threshold,max_threshold,min_t_level_t,max_t_level_t):
        super().__init__()
        self.ee_mode = ee_mode
        self.win_sizes = [3,5,7,9,11,13,15]
        self.thresholds = [0.01,0.03,0.05,0.1,0.15,0.2,0.25]
    ...
    def on_batch_begin(self, batch_x, batch_y, indices):
        ...
        for i in range(self.sandwich_small):
        ...
        for i,old_pred in enumerate(old_pred_batch):
            token_exit_layers = simulate_ee(old_pred,self.ee_mode,
                                            ...
                                            self.device_,constrain_both=self.constrain_both,)
